...
export default class Frame {
  ...
  constructor(widthPx = 800, heightPx = 600, dpr = (typeof window !== 'undefined' ? window.devicePixelRatio || 1 : 1)) {
    this.dpr = dpr;
  ...
  samplePixel(x, y) {
    ...
    if (sx < 0 || sy < 0 || sx >= this.canvas.width || sy >= this.canvas.height) return [0,0,0,0];
  ...
  fill(x, y, colorRGBA) {
    ...
    if (tr === colorRGBA[0] && tg === colorRGBA[1] && tb === colorRGBA[2] && ta === colorRGBA[3]) return;
    const stack = [[sx, sy]];
    ...
    const replR = colorRGBA[0], replG = colorRGBA[1], replB = colorRGBA[2], replA = colorRGBA[3];
    const mask = new Uint8Array(w * h);
    for (let py = 0; py < h; py++) {
      for (let px = 0; px < w; px++) {
        const i = (py * w + px) * 4;
        if (data[i] === replR && data[i+1] === replG && data[i+2] === replB && data[i+3] === replA) {
          mask[py * w + px] = 1;
        }
      }
    }
