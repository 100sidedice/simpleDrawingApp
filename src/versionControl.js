...
export default class VersionControl {
  ...
  // commit(frame, message) - stores a snapshot and truncates future commits
  commit(frame, message = '') {
    ...
    const commit = { id: String(Date.now()) + '-' + Math.random().toString(36).slice(2, 8), ts: Date.now(), message, image: copy };
  ...
  getCommits() {
    return this.commits.map(({ id, ts, message }, i) => ({ id, ts, message, index: i }));
  }
}
